...
(defn create-product!
  "sku, name, price-cents are required. description/images are optional."
  [{:keys [sku name price-cents description images]}]
  ...
  (let [tx-data [(cond-> {:product/sku sku
                   ...
                   images      (assoc :product/images images))]]
...
(defn get-product [db sku]
  (d/pull db '[:product/sku :product/name :product/description
               :product/price-cents :product/images :product/active?]
          [:product/sku sku]))
...
(defn list-active-products
  ...
  (->> (d/q '[:find (pull ?p [:product/sku :product/name
                               :product/price-cents :product/images])
              :in $

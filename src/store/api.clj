...
(defn create-checkout-session-handler
  "Note: the actual Stripe webhook route lives outside this reitit app
   entirely — see store.stripe/wrap-webhook and store.core — because it
   needs the raw request body, not the JSON-parsed one this router hands
   every other handler."
  [{:keys [path-params body-params]}]
  (try
    (let [order-id (Long/parseLong (:id path-params))]
      {:status 200 :body (stripe/create-checkout-session! order-id body-params)})
    (catch Exception e
      {:status 400 :body {:error (ex-message e)}})))
...
(def routes
  [["/api/products"
   ...
   ["/api/orders/:id/checkout-session"
    {:post {:handler create-checkout-session-handler}}]

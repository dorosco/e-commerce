...
- `src/store/products.clj` — create/update/deactivate/query functions
  for products
- `src/store/api.clj` — reitit routes and handlers exposing products
  over HTTP/JSON
...
## Not done yet (by design — this is just Phase 1's first slice)

- No auth on the write endpoints (`POST`/`PUT`/`DELETE` on `/api/products`)
  — needs middleware before anything here is public
- No inventory, orders, customers, or Stripe integration yet — those
  follow the same pattern (schema → transaction fns → routes) once the
  product catalog feels solid
